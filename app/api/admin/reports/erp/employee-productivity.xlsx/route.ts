...
import { employeeProductivityXlsx } from "@/lib/reports/erp/xlsx";
...
export async function GET(req: Request) {
  ...
  try {
    ...
    const buf = await employeeProductivityXlsx(report, session.user.name ?? "Admin");
    return new NextResponse(Buffer.from(buf) as unknown as BodyInit, {
      headers: {
        "Content-Type":
          "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
        "Content-Disposition": `attachment; filename="Mitarbeiter-Produktivitaet_${fromIso}_${toIso}.xlsx"`,
      },
    });
  } catch (err) {
    console.error("[employee-productivity.xlsx] failed:", err);
